...
namespace Hyland.Server.Updater.OnBase.CLI.Handlers
{
    internal sealed class UpdateHandler
    {
        ...
        public UpdateHandler(IRepository<Application> applicationRepo, IPackageRepository packageRepo, IUpdateService updateService, IApplicationFileInformationService fileInfoService)
        {
        ...
        public void HandleRequest(ParseResult parseResult)
        {
            try
            {
                ...
                _updateService.InstallUpdate(application, package);

                Console.WriteLine("Successfully installed update.");
            }
            catch (Exception e)
            {
                Console.WriteLine("Unexpected error installing update.");
                Console.WriteLine(e);
            }
        }
    }
